...
def insertar_datos_en_db(db_params, clave, datos_tabla):
    try:
        ...
        for row in datos_tabla:
            ...
            # Si fecha_firma no contiene el año_clave, omitir la fila
            if año_clave not in fecha_firma:
                continue
...
for clave in claves[:expedientes_a_procesar]:
    ...
    # Obtener el último ID ocupado antes de procesar la clave
    ...
    # Solo ejecutar la función si se activan las descargas
    if ejecutar_descargas:
        asociar_archivos_a_registros(db_params, datos_tabla, renombrados)  # Pasa la lista renombrados como argumento
        renombrados.clear()  # Limpiar la lista para la próxima clav
